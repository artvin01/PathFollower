...
static cell_t PF_UpdateLastKnownArea(IPluginContext *pCtx, const cell_t *params)
{
	...
	const auto goal_bcc = static_cast<CBaseCombatCharacter*>(goal_ent);
...
static cell_t PF_Create(IPluginContext *pCtx, const cell_t *params)
{
	...
	const float        step_height         = sp_ctof(params[2]);
	const float        max_jump_height     = sp_ctof(params[3]);
	const float        death_drop_height   = sp_ctof(params[4]);
	const float        slope_limit         = sp_ctof(params[5]);
	const unsigned int body_solid_mask     = params[6];
	const float        look_ahead_distance = sp_ctof(params[7]);
	const float        repath_interval     = sp_ctof(params[8]);
	...
	const auto loco = static_cast<PluginBotLocomotion*>(ctx.m_pPluginBot->GetLocomotionInterface());
	...
	const auto body = static_cast<PluginBotBody*>(ctx.m_pPluginBot->GetBodyInterface());
...
static cell_t PF_Destroy(IPluginContext *pCtx, const cell_t *params)
{
	...
	const auto it = bots.find(bot_entidx);
	bots.erase(it);
...
static cell_t PF_GetGoalVector(IPluginContext *pCtx, const cell_t *params)
{
	...
	const PFContext &ctx = bots.at(bot_entidx);
...
static cell_t PF_IsPathToVectorPossible(IPluginContext *pCtx, const cell_t *params)
{
	...
	const bool success = temp_path->Compute(ctx.m_pPluginBot, vec, *cost_func);
...
static cell_t PF_IsPathToEntityPossible(IPluginContext *pCtx, const cell_t *params)
{
	...
	const bool success = temp_path->Compute(ctx.m_pPluginBot, goal_bcc, *cost_func);
...
static cell_t PF_IsPotentiallyTraversable(IPluginContext *pCtx, const cell_t *params)
{
	...
	float      fraction;
	const bool bResult = ctx.m_pPluginBot->GetLocomotionInterface()->IsPotentiallyTraversable(from, to, when, &fraction);
...
static cell_t PF_HasPotentialGap(IPluginContext *pCtx, const cell_t *params)
{
	...
	float      fraction;
	const bool bResult = ctx.m_pPluginBot->GetLocomotionInterface()->HasPotentialGap(from, to, &fraction);
...
{
	const auto area = (const CNavArea*)params[1];
...
{
	const auto  area = (const CNavArea*)params[1];
	const float x    = sp_ctof(params[2]);
	const float y    = sp_ctof(params[3]);
...
static cell_t NavArea_ComputeNormal(IPluginContext *pCtx, const cell_t *params)
{
	...
	const bool alternate = (params[3] != 0);
...
{
	const auto          area   = (const CNavArea*)params[1];
	const NavCornerType corner = (NavCornerType)params[3];
...
{
	const auto       area = (const CNavArea*)params[1];
	const NavDirType dir  = (NavDirType)params[2];
...
{
	const auto area            = (const CNavArea*)params[1];
	const auto destinationArea = (const CNavArea*)params[2];
...
static cell_t NavMesh_GetNavArea_Vec(IPluginContext *pCtx, const cell_t *params)
{
	...
	const Vector pos          = GetVector(params[2], pCtx);
	const float  beneathLimit = sp_ctof(params[3]);
...
static cell_t NavMesh_GetNearestNavArea_Vec(IPluginContext *pCtx, const cell_t *params)
{
	auto         mesh        = (const CNavMesh*)params[1];
	const Vector pos         = GetVector(params[2], pCtx);
	const bool   anyZ        = (params[3] != 0);
	const float  maxDist     = sp_ctof(params[4]);
	const bool   checkLOS    = (params[5] != 0);
	const bool   checkGround = (params[6] != 0);
	const int    team        = params[7];
...
static cell_t NavMesh_NavAreaTravelDistance(IPluginContext *pCtx, const cell_t *params)
{
	...
	return sp_ctof(NavAreaTravelDistance(mesh_1, mesh_2, function, maxDist));
}
...
const sp_nativeinfo_t g_Natives[] = {
	...
	{"NavAreaTravelDistance", &NavMesh_NavAreaTravelDistance},
